...
func render() -> void:
	...
	var characters: Array[ClientCharacterSummary] = _get_characters()
	...
	var visible_count: int = mini(characters.size(), mini(MAX_VISIBLE_CHARACTERS, _row_slots.size()))
	...
	for index: int in range(_row_slots.size()):
		...
		if index < visible_count:
			var character: ClientCharacterSummary = characters[index]
...
func _select_character(index: int) -> void:
	var characters: Array[ClientCharacterSummary] = _get_characters()
	if index < 0 or index >= characters.size() or index >= _row_slots.size():
		return
...
func _get_characters() -> Array[ClientCharacterSummary]:
	return characters
