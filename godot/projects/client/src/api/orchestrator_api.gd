...
const TYPE_ZONE_CHANGE_FAILURE: String = "zone_change_failure"
...
func request_character_login(character_id: int) -> Dictionary:
	print("Client requesting character login zone token: character_id=%d" % character_id)
	var err: Error = await _ensure_connected()
	if err != OK:
		return _failure("Could not connect to orchestrator: %s" % error_string(err))

	if not _begin_request("character_login"):
		return _failure("Orchestrator request already in progress.")
	...
	return await _wait_for_request("character_login")

func request_zone_change(target_zone_id: String, details: Dictionary = {}) -> Dictionary:
	print("Client requesting zone change token: zone_id=%s" % target_zone_id)
	var err: Error = await _ensure_connected()
	if err != OK:
		return _failure("Could not connect to orchestrator: %s" % error_string(err))

	if not _begin_request("zone_change"):
		return _failure("Orchestrator request already in progress.")

	err = _send({
		"type": "zone_change_request",
		"target_zone_id": target_zone_id,
		"details": details,
	})
	if err != OK:
		_clear_request()
		return _failure("Could not send zone change request: %s" % error_string(err))

	return await _wait_for_request("zone_change")

func request_character_select(character_id: int) -> Dictionary:
	return await request_character_login(character_id)
...
func _handle_message(message: Dictionary) -> void:
	...
	match message_type:
		...
		TYPE_CHARACTER_SELECT_FAILURE:
			_finish_zone_token_request(_failure(str(message.get("reason", "Character select failed."))))
		TYPE_ZONE_CHANGE_FAILURE:
			_finish_zone_token_request(_failure(str(message.get("reason", "Zone change failed."))))
		TYPE_ZONE_REDIRECT:
...
func _finish_zone_token_request(result: Dictionary) -> void:
	if _active_request_type != "character_login" and _active_request_type != "zone_change" and _active_request_type != "character_select":
		return
	_active_request_type = ""
	_active_request_result = result
