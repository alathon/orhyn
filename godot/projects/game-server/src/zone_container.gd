...
@export var config: GameServerConfig
...
func load_zone(zone_id: String) -> void:
	...
	if normalized_zone_id.is_empty():
		push_error("Cannot load zone with an empty zone id.")
		return

	var scene_path: String = _get_zone_scene_path(normalized_zone_id)
	var loaded_resource: Resource = ResourceLoader.load(scene_path)
	var zone_scene: PackedScene = loaded_resource as PackedScene
	if zone_scene == null:
		push_error("Failed to load zone scene for '%s' at %s." % [normalized_zone_id, scene_path])
		return
	...
	if entities == null:
		push_error("Loaded zone '%s' does not have an Entities node." % normalized_zone_id)
		zone.queue_free()
		return
	...
	print("Loaded zone '%s' from %s" % [loaded_zone_id, scene_path])
...
func _get_zone_scene_path(zone_id: String) -> String:
	return "res://projects/zones/zone_%s/zone_%s.tscn" % [zone_id, zone_id]
